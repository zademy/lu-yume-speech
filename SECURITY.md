...
| Area                      | Detail                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **API key storage**       | Credentials are stored in the browser's `localStorage` via the `WebBridge` platform implementation: the Groq API key under `stt_groq_api_key`, the Cloudflare worker token under `stt_worker_token`, and the MiniMax Speech-to-Text key under `stt_minimax_api_key` (each provider uses its own independent credential). They are **never** written to `sessionStorage` or the JS bundle. Be aware that browser-side storage is only as secure as the origin serving the app.                                                                                                                             |
...
| **Network egress**        | When deploying, set a `Content-Security-Policy` such as `default-src 'self'; script-src 'self' 'wasm-unsafe-eval'; worker-src 'self'; connect-src 'self' https://api.groq.com https://api.minimax.io https://huggingface.co https://us.aws.cdn.hf.co https://eu.aws.cdn.hf.co https://cdn-lfs.huggingface.co https://cdn-lfs-us-1.huggingface.co https://cdn-lfs-eu-1.huggingface.co; font-src 'self' data:; style-src 'self' 'unsafe-inline'; img-src 'self' data:; object-src 'none'; base-uri 'self'; frame-ancestors 'none'` (extend with every remote origin in use — e.g. a configured Cloudflare worker). `api.minimax.io` serves the MiniMax Speech-to-Text provider (`/v1/speech_to_text`, Bearer key from browser storage like the Groq key). `huggingface.co` `resolve/` URLs redirect per-request to the Xet/LFS CDN hosts (`*.aws.cdn.hf.co`, `cdn-lfs*.huggingface.co`) with short-lived signatures, so those must be allowlisted too. `wasm-unsafe-eval` is required to compile the same-origin ONNX Runtime WASM binaries; fonts are self-hosted via `@fontsource-variable` (no third-party font CDN); the theme bootstrap ships as an external same-origin script, so **no inline scripts, no eval, no third-party origins**.                                                                                                                                     |
...
| **Local inference**      | Transcripción local runs Transformers.js + ONNX Runtime Web entirely inside a same-origin module Web Worker (CSP: `worker-src 'self'`, no blob workers). Model weights are served from the app's own Cache API store (a cache miss is an error, never a silent network download), and ORT's WASM binaries are bundled and served from the app's own origin — no CDN egress at inference time. Cancellation terminates the worker; downloaded weights are untouched. |
| **Local benchmark**      | The in-app Diagnóstico (T8) measures models against a synthetic corpus served from the app's **own origin** (`public/benchmark-corpus/`); it makes no network requests and never measures user audio. Published numbers live in `src/utils/benchmark/results.json` (aggregate, no user data). See `docs/benchmark.md`. |
