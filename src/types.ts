...
/** One model response generated from a transcription snapshot. */
export interface GeneratedSummary {
  /** Unique identifier (crypto.randomUUID) */
  id: string;
  /** Short prose overview in the transcription language. */
  summary: string;
  /** Main ideas extracted from the transcription. */
  keyPoints: string[];
  /** Groq chat model used. */
  model: string;
  /** Unix timestamp in milliseconds. */
  createdAt: number;
}

/** Summaries generated for one exact snapshot of visible transcription text. */
export interface SummaryHistory {
  /** Unique identifier (crypto.randomUUID). */
  id: string;
  /** Exact visible text used as source for every contained summary. */
  sourceText: string;
  /** Generations ordered from oldest to newest. */
  summaries: GeneratedSummary[];
  /** Unix timestamp in milliseconds for the first generation. */
  createdAt: number;
  /** Unix timestamp in milliseconds for the latest generation. */
  updatedAt: number;
}
